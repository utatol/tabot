...
global.prefix = new RegExp('^[' + (opts['prefix'] || '‎xzXZ/i!#$%+£¢€¥^°=¶∆×÷π√✓©®:;U?&.\\-').replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']')
...
if (!global.DATABASE._data.chats) global.DATABASE._data.chats = {}
if (!global.DATABASE._data.stats) global.DATABASE._data.stats = {}
if (!global.DATABASE._data.msgs) global.DATABASE._data.msgs = {}
if (!global.DATABASE._data.sticker) global.DATABASE._data.sticker = {}
...
if (!opts['test']) setInterval(() => {
  ...
  if (JSON.stringify(global.DATABASE._data) == lastJSON) conn.logger.info('Database is up to date')
  else {
    global.DATABASE.save()
    conn.logger.info('Done saving database!')
    lastJSON = JSON.stringify(global.DATABASE.data)
  }
}, 300 * 1000) // Save every minute
...
global.reloadHandler = function () {
  ...
  conn.welcome = 'Hai, @user!\nSelamat datang semoga betah ya^_^'
  conn.bye = 'Dadah🗿 @user '
