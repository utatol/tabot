    ...
    if (args.length < 3) {
        ...
        switch (type) {
            ...
            case 'mythic':
                ...
                break
            case 'limit':
                if (global.DATABASE._data.users[m.sender].limit >= count * 1) {
                    try {
                        global.DATABASE._data.users[m.sender].limit -= count * 1
                        global.DATABASE._data.users[who].limit += count * 1
                        conn.reply(m.chat, `Berhasil mentransfer ${count} Limit`.trim(), m)
                    } catch (e) {
                        global.DATABASE._data.users[m.sender].limit += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                } else conn.reply(m.chat, `Limit kamu gak cukup`.trim(), m)
                break
            case 'legendary':
                ...
                break
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer (barang) (jumlah) (@tag)\ncontoh penggunaan: *${usedPrefix}transfer money 100 @uta_kamgun*\n\n*List yang bisa di transfer*\nMoney\nPotion\nSampah\nDiamond\nCommon\nUncommon\nMythic\nLegendary\nLimit`.trim(), m)
        }
    } catch (e) {
        conn.reply(m.chat, `Format yang anda gunakan salah\n\nGunakan format ${usedPrefix}transfer (barang) (jumlah) <@tag>\ncontoh penggunaan: *${usedPrefix}transfer money 100 @uta_kamgun\n*List yang bisa di transfer*\nMoney\nPotion\nSampah\nDiamond\nCommon\nLimit\nUncommon\nMythic\nLegendary*`.trim(), m)
        console.log(e)
        if (DevMode) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.sendMessage(jid, 'Transfer.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', MessageType.text)
...
handler.tags = ['about']
handler.command = /^(transfer|give|bagi)$/i
handler.owner = false
handler.mods = false
