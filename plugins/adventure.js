...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        let _timers = (200000 - __timers) 
        let timers = clockString(_timers)
        if (global.DATABASE._data.users[m.sender].healt > 49) {
            if (new Date - global.DATABASE._data.users[m.sender].lastadventure > 200000) {
            ...
            let healt = (kucing == 0 && armor == 0 ? pickRandom(['35', '26', '98', '97', '96', '10', '46', '60', '50', '93', '29', '19', '1']) : kucing > 0 && armor > 0 ? __health : ___health)
            let exp = (Math.floor(Math.random() * 7000) + (kuda * 70))
            let uang = `${Math.floor(Math.random() * 5000)}`.trim() 
            ...
            let iron = `${Math.floor(Math.random() *30)}`.trim()
            ...
            let str = `ㅤㅤㅤㅤ「 *ADVENTURE* 」
Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Jepang', 'Korea', 'Bali', 'Amerika', 'Iraq', 'Arab', 'Pakistan', 'German', 'Finlandia', 'Ke bawa dunia mimpi', 'Ujung dunia', 'Mars', 'Bulan', 'Pluto', 'Matahari', 'Hatinya dia', '...', 'Papua', 'Jawa', 'Grup Neko Lovers', 'Kota Badut', 'Ke Grup Badut', 'Ke Game 8Bit', 'Ke Game Papji', 'Jauh Arab keknya', 'Ke Kampung Gunung', 'Jakarta','Bandara', 'Ke Pemakaman'])}
━━━━━━━━━━━━━━━━━
 *↓MENDAPATKAN ITEM↓*
*> Exp💪* +${exp} 
*> Duit💵* +${uang}
...
`.trim()
            ...
            } else conn.reply(m.chat, `Anda Sudah Berpetualang Dan Lelah,  ${pickRandom(['Sampai Lupa Lu Itu Cuma Bahan Gabutan', 'Mending Minum Dulu', 'Liat SW Doi Dulu Biar Gk Gabut', 'Bentar Isitirahat Dulu'])} Cooldown *${timers}* Lagi`, m)
        } else conn.reply(m.chat, 'Minimal 50 Health Untuk Bisa Berpetualang, Ke Shop Dulu Gih Beli Minuman Atau Gak Ketik #claim', m)
    } catch (e) {
        console.log(e)
